...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:x="http://www.w3.org/1999/xhtml" exclude-result-prefixes="#default x">
	...
	<xsl:variable name="sLinkedFilesPath" select="translate(//x:meta[@name='linkedFilesRootDir']/@content,'\','/')"/>
...
-->
	<xsl:template match="@class" priority="100">
		...
		<xsl:if test="$sNewClassName='Picture_Group'">
			<xsl:variable name="pictureHeight" select="parent::*/following-sibling::x:span[@class='senses']/descendant::x:span[@class='LexSense-publishStem-Altura--de--la--imagen']/x:span[1]"/>
...
-->
	<xsl:template match="x:span[@class='complexform-form']">
	...
	<xsl:template match="x:span[starts-with(@class,'crossrefs')]">
		<!-- only need the second span which has the contents -->
		<!--		<xsl:apply-templates select="x:span[string-length(@class)&gt;0]"/>-->
		<xsl:apply-templates/>
	</xsl:template>
	<xsl:template match="x:span[starts-with(@class,'crossref-type')]">
	...
	<xsl:template match="x:span[@class='encyclopedic-info']">
	...
	<xsl:template match="x:span[@class='headword' or @class='headword-minor']">
	...
	<xsl:template match="x:span[@class='LexSense-publishStem-Comentario--restrictivo']">
	...
	<xsl:template match="x:span[@class='primaryrefs-minor']">
	...
	<xsl:template match="x:span[@class='scientific-name']">
	...
	<xsl:template match="x:span[starts-with(@class,'translation') and @class!='translations']">
		<span class="Translation" lang="{@lang}">
			<!-- as long as we catch the ones containing just spaces, we can apply all of them -->
			<xsl:apply-templates/>
		</span>
	</xsl:template>
	<xsl:template match="x:span[@class='sense' and count(preceding-sibling:: x:span[@class='sense'])+count(following-sibling:: x:span[@class='sense'])=0]">
	...
	<xsl:template match="x:span[@class='xitem' and parent::x:span[starts-with(@class,'primaryrefs-minor')] and preceding-sibling::x:span[@class='xitem']]">
		<xsl:variable name="precedingType"
			select="preceding-sibling::x:span[@class='xitem'][1]/descendant::x:span[starts-with(@class,'LexEntryType-publishStemMinorEntryType')]"/>
